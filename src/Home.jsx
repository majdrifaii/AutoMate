        ...
        <div className="max-w-7xl mx-auto px-4 py-7 relative z-10">
          <div className={`max-w-2xl text-white ${isRTL ? 'mr-0 ml-auto' : ''}`}>
            ...
            <p className={`text-xl mb-4 ${isRTL ? 'font-arabic' : ''}`}>
              ...
              {translations[currentLang].hero.subtitle}</p>
            <div className="bg-white/10 border border-white/20 rounded-lg p-4 mb-8">
              <p className={`text-lg mb-2 ${isRTL ? 'font-arabic' : ''}`}>
                <FaGift className={`inline ${isRTL ? 'ml-2' : 'mr-2'} text-accent`} />                {translations[currentLang].hero.consultation}
              </p>
    ...
    <Slider {...Settings}>
      {portfolioSlides[currentLang].map((slide, index) => (
        <div className="relative w-full rounded-lg overflow-hidden" key={index}>
          {/* Text Container - Now Centered Vertically on Small Screens */}
          <div
            className={`absolute top-1/2 -translate-y-1/2 sm:top-10 sm:translate-y-0 
              w-10/12 sm:w-3/4 md:w-1/3 px-4 sm:px-6 
              left-1/2 -translate-x-1/2 sm:left-10 sm:translate-x-0
              text-center sm:text-left 
              bg-white/10 backdrop-blur-[2px] rounded-lg p-3 sm:bg-transparent sm:backdrop-blur-none`}
            dir={isRTL ? 'rtl' : 'ltr'}
          >
            ...
            <div className="bg-white/10 rounded-lg p-6 mb-12 max-w-2xl mx-auto">
              <div className={`flex items-start ${currentLang === 'ar' ? 'space-x-reverse' : 'space-x-4'}`}>
                <div className="text-white">
                  <FaGift className="text-accent text-3xl" />
                </div>
          ...
          <div className="max-w-5xl mx-auto">
            {faqs[currentLang].map((faq, index) => (
              <div key={index} className="mb-4 shadow-md rounded-lg">
                ...
                <div
                  className={`overflow-hidden transition-all duration-700 ease-in-out ${activeAccordion === index
